...
public class PerfTest {
    ...
    private Graph chain() {
        Graph g = new Graph();
        for (int i = 1; i <= 30; i++) g.addEdge("V"+i, "V"+(i+1), i);
        return g;
    }
    ...
    void perfAndConsistency() {
        ...
        // время неотрицательное (мс)
        assertTrue(ms1 >= 0 && ms2 >= 0 && ms3 >= 0 && ms4 >= 0);

        // суммарные операции неотрицательны
        assertTrue(c1.total() >= 0 && c2.total() >= 0 && c3.total() >= 0 && c4.total() >= 0);

        // ИСПОЛЬЗУЕМ ГЕТТЕРЫ: разбиение по операциям тоже неотрицательно
        assertTrue(c1.getComparisons() >= 0);
        assertTrue(c1.getHeapOps() >= 0);
        assertTrue(c2.getComparisons() >= 0);
        assertTrue(c2.getHeapOps() >= 0);
        assertTrue(c3.getUfFind() >= 0);
        assertTrue(c3.getUfUnion() >= 0);
        assertTrue(c4.getUfFind() >= 0);
        assertTrue(c4.getUfUnion() >= 0);
